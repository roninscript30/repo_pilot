...
- **Phase 1.5 Slice 5 "Branch explorer + interactive branch graph" (committed)**: `BranchGraph.tsx` (SVG canvas over `git_commit_graph` DAG; `graph-layout.ts` assigns mainline/side lanes + merge connectors from `parents`; wheel-zoom at cursor (non-passive listener), drag pan, zoom buttons + fit; per-lane colors, merge curves, ref chips branch/remote/tag/HEAD; click commit → select, click chip → inline Checkout/Rename/Delete actions in the header; `CommitInspector` inline below the canvas). Port `GitRuntime.getCommitGraph(path, limit)` + `useCommitGraph` hook (default 2000-node cap). Tests: 4 graph-layout unit tests → 147 Vitest total; lint/typecheck clean; e2e 8/10 (same pre-existing signin flakes).
