...
Phase 0 foundation complete; Phase 1 UI/UX build complete (initial application commit `b2cd37b`); feature-module Repository IDE refactor complete and committed (`199b3d6`) along with the Repo Pilot rebrand and the frontend split into its own `frontend/` npm workspace. **Phase 1.5 in progress: Slices 1-4 are complete and committed — Slice 1 "Account platform" (`7a6e21f`, ADR-0012), Slice 2 "Native Git engine" (`50e739e`, system-git clone/network + all GitOperations + commit graph + file watching, ADRs 0013/0014), Slice 3 "Commit center + sync center" (`42f032b`, commit templates/validation/preview/amend/empty/signed/commit&push/sync, SyncCenter full layout + per-remote account, Rust `commit_to_detail` stats+patch), Slice 4 "Compare center + diff experience" (`aa2d3a7`, structured hunks, multi-ref compare incl. working tree, unified/split/word DiffViewer, merge+rebase preview, ADR-0015). Slice 5 "Branch explorer + interactive branch graph" is next.**
...
- **Phase 1.5 Slice 2 "Native Git engine" (ADRs 0013/0014, committed `50e739e`)**: system-git clone/fetch/pull/push + all GitOperations (restore/checkout/rename-branch/reset/tag/stash/cherry-pick/revert/rebase/merge/squash/compare, commit amend/empty/signed) via `src-tauri/src/git_system.rs`; `git_commit_graph` (full DAG + refs + parents) via `src-tauri/src/git_graph.rs`; `git://progress` + `git://repo-changed` events with a notify `WatchRegistry`; `CommitSummary.parents` populated; frontend `cloneRepository`/`getGitVersion` port methods, `services/git-events.ts`, `features/git/lib/payloads.ts` (20 builders), hooks (`useCloneRepository`/`useGitProgress`/`useRepoChanged`/`useGitVersion`), `CloneRepositoryDialog` wired into LocalReposPage + LocalRepoGate, `features/local/lib/clone.ts` URL helpers.
- **Phase 1.5 Slice 3 "Commit center + Sync center" (committed)**: `CommitCenter` (message editor with persisted templates + validation: empty/72-char subject/WIP hint; live staged preview + per-file stats; amend/empty/signed toggles; Commit / Commit & Push / Commit & Sync via sequential mutations), `SyncCenter` full layout (current/tracking branch, remote, ahead/behind counts, divergence note, per-remote account picker backed by new `LocalRepository.accountLogins`, set-upstream push, Fetch/Pull/Push/Sync/Refresh/Compare with `git://progress`), `CommitsExplorer` (branch filter + search + pagination + local/remote commit modes), `CommitInspector` local-path mode; Rust `commit_to_detail` rewritten to populate additions/deletions/patch (via `diff_tree_to_tree` + `line_stats` + `render_unified_patch`, 2 new tests incl. root-commit all-additions); commit op routes to system git when `signed || amend || empty`. Tests now 135 Vitest (16 files, incl. commit lib + local-store `accountLogins`) + 10 e2e + 10 Rust unit tests.
- **Phase 1.5 Slice 4 "Compare center + diff experience" (committed `aa2d3a7`, ADR-0015)**: `src-tauri/src/diff.rs` with `structured_diff` (Myers hunks, single source of truth; `patch_from_hunks` derives patch text), `git_diff_files` (any two refs: branches/tags/revs/worktree via extended `resolve_commit_id`), `git_tag_list`; `FileDiff.hunks` on worktree diffs too. `CompareView` rewritten: ref kind pickers (branch/tag/commit/worktree), quick compares (working-tree↔HEAD, HEAD↔previous, local↔remote), grouped file tree navigator + structured DiffViewer pane, divergence/stats cards, and a Merge/Rebase preview toggle. `DiffViewer` gained unified/split/word modes, highlight.js `SyntaxHighlight`, markdown preview, hunk collapse, change navigation; `lib/word-diff.ts` char-level LCS word diff (3 tests) and `lib/language.ts` language detection. Tests now 143 Vitest (17 files) + 16 Rust (incl. structured-diff suite); e2e local spec green, 2 signin specs fail on clean HEAD too (environmental, mocked-route drift — not caused by slices).
...
- Slice 5: Branch explorer + interactive branch graph (GitKraken-style canvas over `git_commit_graph` data, zoom/pan/mini-map/click-through); Slice 6: PR + Issues workspaces with Provider mutations (create/edit/review/merge/comment/labels); Slice 7: repo activity + persistent per-account tabs + repo sync sources.
