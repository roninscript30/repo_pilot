...
function GridCard({ repository }: { repository: Repository }) {
  ...
  return (
    <HoverCard trigger={
      <Link to={`/repositories/${repository.fullName}`} className="group flex h-full flex-col rounded-xl border border-surface-200 bg-surface-0 p-4 shadow-card transition-all hover:border-accent-300 hover:shadow-pop dark:border-surface-600 dark:bg-surface-50 dark:hover:border-accent-700">
        <div className="flex items-start gap-2.5">
          <Avatar name={repository.owner.login} src={repository.owner.avatarUrl} size="md" />
          <div className="min-w-0 flex-1">
  ...
  return (
    <tr className="group border-b border-surface-100 transition-colors hover:bg-surface-50 dark:border-surface-700 dark:hover:bg-surface-700/40">
      <td className="px-4 py-2.5">
        <Link to={`/repositories/${repository.fullName}`} className="flex items-center gap-2.5">
