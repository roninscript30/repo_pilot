...
import { useSearchCommits } from "@/features/dashboard/repository-data-hooks";
import { useSearchIssues, useSearchPullRequests } from "@/features/collaboration-hooks";
  ...
  const items = useMemo<readonly CommandItem[]>(() => {
    ...
    for (const commit of commitSearch.data ?? []) {
      results.push({
        ...
        icon: "gitCommit",
        group: "Commits",
        onSelect: () => navigate(repoWorkspacePathForFullName(fullName, "commits")),
      });
    }
    ...
    for (const issue of issueSearch.data ?? []) {
      results.push({
        ...
        icon: "issue",
        group: "Issues",
        onSelect: () => navigate(repoWorkspacePathForFullName(fullName, "issues")),
      });
    }
    ...
    for (const pull of pullSearch.data ?? []) {
      results.push({
        ...
        icon: "gitMerge",
        group: "Pull requests",
        onSelect: () => navigate(repoWorkspacePathForFullName(fullName, "pulls")),
      });
    }
